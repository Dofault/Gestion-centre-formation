...
/*	--------------------------------------------------------
	...
	//fermeture du flux d'écriture "w" et ouverture d'un flux d'écriture "a"
	//fclose(fres);
	fres = fopen("listeEtudiant.dat","a");
	fprintf(fres,"%-30s %-30s %-100s %2d %2d %4d %2d %8.2f %8.2f %8.2f\n", "Test2", "Ajout2", "Kine", 1, 2, 3000, 1, 4000.30, 2000.30, 500);

	//Ca fonctionne. En ouvrant un fichier en "a" au lieu de "w", on ne supprime pas tout le contenu. Attention que le simple fait d'ouvrir un fichier en "w", même si
	//on n'écrit rien dedans, va tout supprimer... (c'est pour ca que j'ai mis en commentaire ton fres)
